				     ...
				     const struct blk_mq_queue_data *bd)
{
	...
	printk(KERN_LOG_LEVEL "blockdev: req data: pos=%llu cur_bytes=%u dir=%s\n", blk_rq_pos(rq), blk_rq_cur_bytes(rq), direction);
...
static int create_block_device(struct my_block_dev *dev)
{
	...
	if (dev->queue == NULL) {
		printk(KERN_ERR "blockdev: blk_mq_init_queue failed\n");
		err = -ENOMEM;
	...
	if (dev->gd == NULL) {
		printk(KERN_ERR "blockdev: alloc_disk: failure\n");
		err = -ENOMEM;
	...
	err = add_disk(dev->gd);
	if (err) {
		printk(KERN_LOG_LEVEL "add_disk: failure\n");
		return err;
	}
...
static int __init my_block_init(void)
{
	...
	if (err < 0) {
		printk(KERN_LOG_LEVEL "register_blkdev: unable to register\n");
		return err;
	}
