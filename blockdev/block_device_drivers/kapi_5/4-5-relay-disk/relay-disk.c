...
{
	struct bio *bio = bio_alloc(bdev, 1, 0, 0);
	struct page *page;
	char *buf;
	...
	buf = kmap_atomic(page);
	printk(KERN_LOG_LEVEL "relay:  %02x,%02x,%02x\n", buf[0], buf[1], buf[2]);
	kunmap_atomic(buf);
...
static struct block_device *open_disk(char *name)
{
	...
	if (bdev == NULL) {
		printk(KERN_LOG_LEVEL "blkdev_get_by_path\n");
		return NULL;
	}
...
static int __init relay_init(void)
{
	...
	if (phys_bdev == NULL) {
		printk(KERN_LOG_LEVEL "[relay_init] No such device\n");
		return -EINVAL;
	}
